...
## Responsive Content + Ads System

### CTA buttons
`src/data/ads.json` -> `ctaButtons` controls:
- `smartClick`: optional Smart Click CTA; leave `url` empty to hide.
- `refer`: optional Refer & Earn CTA; leave `url` empty to hide.
- The real `Watch Now` button always points to the configured movie `videoEmbedUrl`.

### Content blocks inside a movie JSON
Each movie can optionally contain a `content` array. Supported blocks:
```json
{
  "content": [
    { "type": "heading", "text": "Story Overview" },
    { "type": "paragraph", "text": "Your content..." },
    { "type": "image", "src": "https://example.com/image.jpg", "alt": "Scene" },
    { "type": "link", "url": "https://example.com", "text": "Learn more" },
    { "type": "divider" },
    { "type": "ad", "image": "https://example.com/ad.jpg", "url": "https://example.com" }
  ]
}
```
Use `type: "html"` only for trusted HTML/code that you control.

### Recommended ad inventory
- Desktop: one top banner + one in-content/native unit + optional left/right rail at 1280px+.
- Mobile: one delayed slide ad and/or a small sticky banner, not several overlays at once.
- Side rails: around 160x600 or 300x250, only on large screens.
- Top banner: 728x90 desktop; responsive 320x100 or 320x50 mobile.
- Image ads: always show an `Advertisement` label.
- Do not use fake player/download buttons, deceptive close buttons, or ads that visually imitate navigation.

### Frequency / UX defaults
Mobile slide ads are delayed, auto-close after a short period, and session-capped. Change values in `responsiveAds.mobileSlide`.
